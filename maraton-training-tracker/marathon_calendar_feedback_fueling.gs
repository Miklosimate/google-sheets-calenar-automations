/**
 * Simplified Marathon Plan <-> Google Calendar sync (with fueling)
 *
...
function canonicalFeedbackKey_(key) {
  ...
  var map = {
    ...
    fuel: 'actual_fuel_notes',
    fueling: 'actual_fuel_notes',
    fuel_notes: 'actual_fuel_notes',
    actual_fuel: 'actual_fuel_notes',
    actual_fuel_notes: 'actual_fuel_notes'
  };
...
function hasMeaningfulActuals_(actuals) {
  ...
  var keys = ['actual_km', 'actual_duration_min', 'actual_avg_hr', 'actual_max_hr', 'actual_avg_pace', 'actual_rpe', 'actual_notes', 'actual_fuel_notes'];
...
function buildFeedbackSummary_(rowObj) {
  ...
  if (!isBlankish_(rowObj.actual_fuel_notes)) out.push('Fuel: ' + rowObj.actual_fuel_notes);
...
function buildEventDescription_(rowObj, settings, rowNumber) {
  ...
  if (!isBlankish_(rowObj.planned_fuel_to_bring)) lines.push('bring=' + rowObj.planned_fuel_to_bring);
  if (!isBlankish_(rowObj.planned_fuel_timing)) lines.push('fuel_timing=' + rowObj.planned_fuel_timing);
  lines.push('');
  lines.push('Feedback: edit only the block below. You can use "=" or ":".');
  lines.push('Examples: status=done, km=12, avg_hr=138, fuel=banana + dates, skipped');
  lines.push('');
...
function mergeActualsWithExistingRow_(rowObj, actuals) {
  var merged = {
    ...
    actual_fuel_notes: rowObj.actual_fuel_notes || ''
  };

  if (!actuals) return merged;
  if (normalizeActualStatus_(actuals.status || '') === 'skipped') return skippedPayload_();

  var keys = ['status', 'actual_km', 'actual_duration_min', 'actual_avg_hr', 'actual_max_hr', 'actual_avg_pace', 'actual_rpe', 'actual_notes', 'actual_fuel_notes'];
  for (var i = 0; i < keys.length; i++) {
    var key = keys[i];
...
function setRowValuesFromActuals_(sheet, rowNumber, headers, actualMap) {
  ...
  if (typeof headers.actual_fuel_notes !== 'undefined') {
    var fuelCell = sheet.getRange(rowNumber, headers.actual_fuel_notes + 1);
    fuelCell.setNumberFormat('@STRING@');
    fuelCell.setValue(String(actualMap.actual_fuel_notes || ''));
  }
